...
v1.4.4 regression note: We previously tried wrapping UPX in a shell/bat
script to strip ``--lzma`` (LZMA decompression is ~10x slower than
NRV2E). However, Nuitka's ``--upx-binary`` option expects a DIRECTORY
containing ``upx.exe``, not a file path (Nuitka Issue #1517). The
wrapper script (named ``upx-wrapper.bat``) was not recognized as
``upx.exe``, so Nuitka silently skipped ALL UPX compression — producing
uncompressed binaries that were larger and slower than v1.4.2. The
fix is to NOT use a wrapper and let Nuitka invoke UPX directly with
its default ``--best --lzma`` flags. v1.4.2 proved this configuration
...
def resolve_upx_for_build(target: str, *, enabled: bool) -> str | None:
    ...
    Returns the path to the real UPX binary (NOT a wrapper). Nuitka's
    ``--upx-binary`` option expects either the UPX binary path or the
    directory containing it. We pass the binary path directly; Nuitka
    will invoke it with its default flags (``--best --lzma``).
