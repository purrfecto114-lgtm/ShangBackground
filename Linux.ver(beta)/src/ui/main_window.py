...
class ShangBackgroundWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self._pending_core_action = None
    ...
    def _install_core_log_bridge(self):
        ...
        def _log(msg, *args, **kwargs):
            rendered = None
            try:
                rendered = self._orig_log(msg, *args, **kwargs)
            except TypeError:
                rendered = self._orig_log(msg)
            try:
                level = str(kwargs.get("level", "INFO")).upper()
                display = rendered if rendered is not None else str(msg)
                if level == "ERROR" and "\n" in display:
                    display = display.split("\n", 1)[0] + "  （详细堆栈已写入日志文件/控制台）"
                self.log_signal.emit(str(display))
            except Exception:
                pass
    ...
    def _run_mode_transition(self, label: str, worker_fn):
        ...
        def _worker():
            try:
                ...
                core.save_config()
                if result is False:
                    # worker_fn 返回 False 而非抛异常时，core.log 已记录原因；
    ...
    def _deferred_gui_startup(self):
        ...
        perf = bool(core.config.get("performance_mode", False))

        def _later(delay_ms: int, callback):
            QTimer.singleShot(delay_ms, lambda: self._run_startup_callback(callback))

        self._schedule_preview_refresh(260 if perf else 100)
        _later(900 if perf else 320, self.refresh_bing_cache_list)
        _later(520 if perf else 260, self.apply_native_window_effect)
        _later(720 if perf else 420, lambda: self.create_or_update_tray() if core.config.get("tray_icon", True) else None)
        _later(1700 if perf else 900, self.maybe_show_auto_start_prompt)
        _later(2200 if perf else 1250, self.run_bing_startup_tasks)
        _later(1300 if perf else 950, lambda: self.set_status(t("欢迎使用")) if not self._current_operation_name else None)
    ...
    def _preview_poll_interval(self) -> int:
        return 3000 if bool(core.config.get("performance_mode", False)) else 1200
    ...
    def _refresh_shell_ui_later(self) -> None:
        try:
            ...
            QTimer.singleShot(120, core.refresh_shell_ui)
        except Exception:
            pass
    ...
    def _theme_role_colors(self) -> dict[str, str]:
        if self._theme_is_dark():
            return {
                "bg_main": "#1e1e2e", "bg_widget": "#252536", "bg_input": "#2d2d42",
                "fg_primary": "#e6e6f0", "fg_secondary": "#c4c4d8", "fg_muted": "#9898b8",
                "border": "#4a4a64", "note_bg": "#2d2d42", "danger_bg": "#3b1010",
            }
        return {
            "bg_main": "#f6f8fa", "bg_widget": "#ffffff", "bg_input": "#ffffff",
            "fg_primary": "#24292f", "fg_secondary": "#57606a", "fg_muted": "#6e7781",
    ...
    def _refresh_settings_dialog_surfaces(self) -> None:
        """Keep global settings pages on the themed page surface after live color changes.

        Do not set a local stylesheet on the dialog or scroll viewports here: a
        QWidget/QDialog stylesheet cascades into descendants and can override
        QPushButton text/background resolution after theme-color changes.
        Palette-only refresh keeps the page background stable without touching
        child control rules.
        """
        dlg = getattr(self, "_settings_dialog", None)
        if not self._is_qobject_alive(dlg):
            return
        page_bg = self._theme_role_colors()["bg_main"]
        self._apply_plain_background_palette(dlg, page_bg)
        try:
            for scroll in dlg.findChildren(QScrollArea):
                try:
                    scroll.setStyleSheet("")
                    self._apply_plain_background_palette(scroll, page_bg)
                    viewport = scroll.viewport()
                    if viewport is not None:
                        viewport.setStyleSheet("")
                        self._apply_plain_background_palette(viewport, page_bg)
                    child = scroll.widget()
                    if child is not None:
                        self._apply_plain_background_palette(child, page_bg)
                except RuntimeError:
                    continue
                except Exception:
                    continue
...
QSpinBox:focus, QDoubleSpinBox:focus { border: 2px solid %%visible_accent%%; padding: 2px 23px 2px 7px; }
...
QMenu { background-color: __BG_WIDGET__; color: __FG_PRIMARY__; border: 1px solid __BORDER__; border-radius: 8px; padding: 4px; }
...
"""
    ...
    def _rebuild_stylesheet(self):
        ...
        if dark:
            ...
            _TPL = (
                ...
                f" margin-top: 12px; padding: 16px 12px 12px 12px; background-color: {group_bg}; }}\n"
                f"QGroupBox::title {{ subcontrol-origin: margin; subcontrol-position: top left;"
                f" padding: 2px 10px; left: 10px; color: {fg_primary}; font-size: 13px; font-weight: 700;"
                f" background-color: {group_bg}; border-radius: 4px; }}\n"
                "\n"
                "/* 按钮 */\n"
                ...
                f"QPushButton[settingsAction=\"true\"] {{ background: {bg_input}; color: {fg_primary}; border: 1px solid %%visible_accent%%; border-radius: 6px; padding: 5px 12px; font-size: 13px; font-weight: 500; min-height: 28px; }}\n"
                f"QPushButton[settingsAction=\"true\"]:hover:enabled {{ background: {nav_hover}; border-color: %%pressed_c%%; }}\n"
                f"QPushButton[settingsAction=\"true\"]:pressed:enabled {{ background: {nav_hover}; padding-top: 6px; padding-bottom: 4px; }}\n"
            ...
            _TPL = (
                ...
                "QWidget { background-color: #f6f8fa; color: #24292f; font-family: %%font_family%%; }\n"
                "QLabel { background-color: transparent; }\n"
            "\n"
            "/* 分组框样式 */\n"
            "QGroupBox { font-weight: 600; font-size: 13px; border: 1px solid #d0d7de; border-radius: 8px;"
            " margin-top: 12px; padding: 16px 12px 12px 12px; background-color: #f6f8fa; }\n"
            "QGroupBox::title { subcontrol-origin: margin; subcontrol-position: top left;"
            " padding: 2px 10px; left: 10px; color: #24292f; font-size: 13px; font-weight: 700; background-color: #f6f8fa; border-radius: 4px; }\n"
            "\n"
            ...
            "QPushButton[settingsAction=\"true\"] { background: #ffffff; color: #24292f; border: 1px solid %%visible_accent%%; border-radius: 6px; padding: 5px 12px; font-size: 13px; font-weight: 500; min-height: 28px; }\n"
            "QPushButton[settingsAction=\"true\"]:hover:enabled { background: #f6f8fa; border-color: %%pressed_c%%; }\n"
            "QPushButton[settingsAction=\"true\"]:pressed:enabled { background: #eaeef2; padding-top: 6px; padding-bottom: 4px; }\n"
            "QPushButton[settingsAction=\"true\"]:disabled { background: #f6f8fa; border-color: #d0d7de; color: #8c959f; }\n"
            "\n"
            "/* 输入框样式 */\n"
            ...
            "\n"
            "/* 滚动区域与滚动条 */\n"
            "QScrollArea, QScrollArea > QWidget, QScrollArea > QWidget > QWidget, QStackedWidget { border: none; background-color: #f6f8fa; }\n"
        ...
        if hasattr(self, "about_sprite_btn"):
            sprite_bg = self._theme_role_colors()["bg_main"]
    ...
    def _wallpaper_tab(self):
        ...
        video_layout.addWidget(self.video_volume_hint_label, 3, 0, 1, 4)
        ...
        video_layout.addWidget(linux_video_note, 4, 0, 1, 4)
        ...
        self._set_button_svg_icon(btn_restart_home, "restart.svg")
        ...
        current_path_row.setSpacing(8)
        current_path_row.addWidget(self.current_label, 1)
        self.btn_jump_current_wallpaper = QPushButton(t("跳转到壁纸"))
        self.btn_jump_current_wallpaper.setProperty("secondary", True)
        self.btn_jump_current_wallpaper.setMinimumHeight(34)
        self.btn_jump_current_wallpaper.clicked.connect(self.open_wallpaper_sidebar)
        current_path_row.addWidget(self.btn_jump_current_wallpaper)
    ...
    def _settings_tab(self):
        ...
        self.perf_mode_check = QCheckBox(t("性能模式（降低后台处理）"))
        self.perf_mode_check.setChecked(bool(core.config.get("performance_mode", False)))
        self.perf_mode_check.toggled.connect(self._on_performance_mode_toggled)
        self.perf_mode_check.setToolTip(t("开启后禁用所有过渡动画，适合低性能设备或减少视觉运动需求。"))
        appearance_form.addRow(t("性能优化"), self.perf_mode_check)
        ...
        def _on_settings_nav(idx):
            ...
            # 刷新面板控件状态（暗色模式切换后可能过时）；不再使用 QGraphicsEffect，避免 Qt painter 警告刷屏。
            QTimer.singleShot(0, self.refresh_from_config)
    ...
    def restore_factory_settings(self):
        ...
        try:
            ...
            QMessageBox.information(self, t("恢复出厂设置"), t("已恢复出厂设置。部分启动项、语言和 DPI 设置可能需要重启程序后完全生效。"))
        except Exception as exc:
            QMessageBox.warning(self, t("恢复出厂设置"), t("恢复出厂设置失败：") + str(exc))
    ...
    def _on_performance_mode_toggled(self, checked: bool) -> None:
        """切换性能模式，立即降低预览刷新、缩略图解码和动画相关开销。"""
        core.config["performance_mode"] = bool(checked)
        core.save_config()
        self._apply_performance_mode_runtime()
        self.set_status(t("性能模式已开启：已降低预览刷新与缩略图处理频率") if checked else t("性能模式已关闭：预览刷新已恢复"))
    ...
    def _bing_tab(self):
        ...
        self.bing_auto_update_count_spin.setFixedWidth(62)
        ...
        self.bing_auto_delete_count_spin.setFixedWidth(62)
        ...
        options_row = QWidget()
        options = QHBoxLayout(options_row)
        options.setContentsMargins(0, 0, 0, 0)
        options.setSpacing(8)
        options.addWidget(QLabel(t("分辨率")))
        options.addWidget(self.bing_resolution)
        options.addWidget(QLabel(t("同步张数")))
        options.addWidget(self.bing_count_spin)
        options.addWidget(QLabel(t("张")))
        options.addSpacing(10)
        options.addWidget(self.bing_auto_update_check)
        options.addWidget(self.bing_auto_update_count_spin)
        options.addWidget(QLabel(t("张")))
        options.addSpacing(10)
        options.addWidget(self.bing_auto_delete_check)
        options.addWidget(self.bing_auto_delete_count_spin)
        options.addWidget(QLabel(t("张")))
        ...
        status_layout.addWidget(self.bing_progress, 2)
        status_layout.addWidget(self.bing_status, 3)
    ...
    def eventFilter(self, obj, event):
        try:
            if isinstance(obj, QListView) and obj.objectName() == "ComboPopupView" and event.type() == QEvent.Type.Show:
                self._animate_widget_flash(obj)
        except Exception:
            pass
    ...
    def refresh_from_config(self):
        ...
        try:
            ...
            if hasattr(self, "perf_mode_check"):
                self.perf_mode_check.blockSignals(True)
                self.perf_mode_check.setChecked(bool(cfg.get("performance_mode", False)))
                self.perf_mode_check.blockSignals(False)
    ...
    def run_core(self, fn, *args):
        ...
        if self._core_busy:
            self._pending_core_action = (fn, args)
            self.set_status(t("已有壁纸操作正在执行，已合并最新请求…"))
            return None
        self._core_busy = True
        try:
            core.clear_cancel_operations()
    ...
    def _on_core_finished(self, ok: bool, message: str, _result):
        ...
        pending = getattr(self, "_pending_core_action", None)
        self._pending_core_action = None
        if pending and not self._core_busy:
            fn, args = pending
    ...
    def start_video_wallpaper_from_gui(self):
        ...
        if not path:
            QMessageBox.information(self, t("视频壁纸"), t("请先选择视频文件"))
            return
        core.config["video_file"] = path
        core.config["mode"] = "视频"
        self._set_combo_current_data(self.mode_combo, "视频")
    ...
    def _schedule_preview_refresh(self, initial_delay: int = 0):
        ...
        def _first_refresh():
            self.update_preview()
            delays = (700,) if bool(core.config.get("performance_mode", False)) else (120, 450, 1000, 1800)
            for delay in delays:
    ...
    def _load_icon_pixmap(self, path: str, size: QSize) -> QPixmap:
        ...
        reader.setAllocationLimit(64 if bool(core.config.get("performance_mode", False)) else 128)
    ...
    def open_wallpaper_sidebar(self) -> None:
        ...
        if _is_linux_wayland_session():
            ok, message = _open_path_in_linux_file_manager(current if current and os.path.exists(current) else folder)
            if not ok:
                QMessageBox.warning(self, t("跳转失败"), message)
            else:
                self.set_status(t("Wayland 下已使用文件管理器打开当前壁纸位置"))
            return
        ...
        def _work():
            try:
                ...
                if self._current_operation_cancel.is_set():
                    ...
                    return
                if set_latest:
                    core.set_wallpaper(latest, t("必应壁纸"))
    ...
    def _format_update_status_text(self, ok: bool, message: str, info: dict, *, source: str = "manual") -> str:
        ...
        asset_text = ""
        if selected_asset:
            size_mb = (selected_asset.get("size") or 0) / 1024 / 1024
            asset_text = f"\n{t('附件：')}{selected_asset.get('name', '')}（{size_mb:.1f} MB）"
