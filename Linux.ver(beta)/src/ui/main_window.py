...
class ShangComboBox(QComboBox):
    ...
    def showPopup(self) -> None:  # noqa: N802 - Qt API
        ...
        # Linux 桌面环境（KDE/GNOME/XFCE）的 Qt 原生主题已经为弹出菜单绘制
        # 圆角和阴影。强制 WA_TranslucentBackground + FramelessWindowHint 会
        # 破坏原生主题，导致圆角不对称、阴影消失。QSS 的 border-radius 只
        # 控制菜单内部背景，窗口形状由系统/Qt 主题处理。
        try:
...
"""
    ...
    def _wallpaper_tab(self):
        ...
        linux_video_note = QLabel(t("Linux 视频壁纸：X11 使用 xwinwrap + mpv；Wayland/wlroots 使用 mpvpaper（需已安装并在 PATH 中）。"))
        linux_video_note.setWordWrap(True)
        linux_video_note.setProperty("muted", True)
        video_layout.addWidget(linux_video_note, 5, 0, 1, 4)
        ...
        # Bug 19 fix: 旧版 note 说"使用 xwinwrap"但实际并未使用 xwinwrap，
        # 只是创建了一个 Qt WebEngine 无边框窗口置底。更新说明并明确限制：
        # Linux 上 HTML 壁纸不会真正嵌入桌面图标层，鼠标穿透仅在 X11 下可用。
        linux_html_note = QLabel(t("Linux HTML 壁纸：创建无边框 Qt WebEngine 窗口置底显示，不会嵌入桌面图标层。鼠标穿透仅在 X11 下可用，Wayland 下可能无效。"))
        linux_html_note.setWordWrap(True)
        linux_html_note.setProperty("muted", True)
        html_layout.addWidget(linux_html_note, 4, 0, 1, 4)
        ...
        btn_restart_home = QPushButton(t("重启程序"))
        btn_restart_home.setProperty("secondary", True)
        self._set_button_svg_icon(btn_restart_home, "restart.svg", size=20)
        btn_restart_home.clicked.connect(self.restart_program)
...
"bing": t("同步必应"), "jump": t("跳转壁纸"), "about": t("关于"), "exit": t("退出"),
        ...
        # v1.4.7: 移除"应用内热键"QGroupBox (与打字冲突, 焦点抢断).
    ...
    def restart_program(self, extra_args=None):
        ...
        try:
            ...
            if self.tray:
                ...
                self._refresh_shell_ui_later()
            self._perform_exit_cleanup_once(restore_wallpaper=False)
            subprocess.Popen(
                cmd,
                cwd=core.BASE_DIR,
                stdout=subprocess.DEVNULL,
