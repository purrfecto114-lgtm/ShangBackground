...
if PYSIDE_AVAILABLE:
    ...
    class UpdateChecker(QThread):
        ...
        def run(self):
            try:
                has_update, info = check_latest_release(core.VERSION)
                sync_legacy_update_state(core, info, has_update)
                self.finished.emit(True, t("发现新版本") if has_update else t("当前已是最新版本"), info.as_dict())
            except Exception as exc:
                sync_legacy_update_state(core, None, False, failed=True)
    ...
    class ShangBackgroundWindow(QMainWindow):
        ...
        def open_random_probability_settings(self):
            ...
            explain = QLabel(t("每张壁纸可以设置概率权重 (0-100)；权重越大，被随机选中的概率越高。实际概率 = 单项权重 ÷ 总权重 × 100%；权重为 0 的壁纸不会被随机选中。拖动滑块或直接输入数值即可调整。"))
        ...
        def start_update_check(self, button=None):
            ...
            self._set_update_status_text("正在检查统一 GitHub Release 更新源...")
