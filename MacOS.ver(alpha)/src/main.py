...
if PYSIDE_AVAILABLE:
    ...
    class UpdateChecker(QThread):
        ...
        def _sync_legacy_update_state(self, latest_version: str, notes: str, assets: list, has_update: bool, failed: bool = False):
            """把旧版内置更新变量同步到 GitHub Release，避免两个更新源显示不一致。"""
            try:
                ...
                core.remote_download_urls = {"GitHub Release": first_asset, t("发布页"): GITHUB_LATEST_RELEASE_URL}
        ...
        def run(self):
            ...
            try:
                ...
                has_update = self._parse_version(latest_version) > self._parse_version(core.VERSION)
                self._sync_legacy_update_state(latest_version, info["body"], assets, has_update)
                self.finished.emit(True, t("发现新版本") if has_update else t("当前已是最新版本"), info)
            except Exception as exc:
                self._sync_legacy_update_state("", "", [], False, failed=True)
                self.finished.emit(False, f"检查更新失败：{exc}", {})
