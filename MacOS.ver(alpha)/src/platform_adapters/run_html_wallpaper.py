"""HTML wallpaper runtime — macOS (v6, refactored).

Renders a local or remote web page as a borderless window on the macOS
desktop.  Accepts ``--path`` plus several option flags.
...
3. **Desktop layer + mouse-through** — bridges the Qt NSView to its NSWindow,
   places it below Finder's desktop-icon level, and toggles mouse events.
...
The process exits instead of falling back to a normal application window when
the native NSWindow bridge cannot be established.
"""
...
# ============================================================================
# Section 4 — Mouse-through (AppleScript via NSWindow)
# ============================================================================

def _native_macos_window(view: QWebEngineView):
    import ctypes
    import objc
    native_view = objc.objc_object(c_void_p=ctypes.c_void_p(int(view.winId())))
    return native_view.window()


def _configure_macos_desktop_window(view: QWebEngineView, mouse_through: bool) -> bool:
    """Place the Qt window below Finder icons using the native NSWindow level."""
    try:
        import AppKit
        import Quartz
        native_window = _native_macos_window(view)
        if native_window is None:
            return False
        icon_level = int(Quartz.CGWindowLevelForKey(Quartz.kCGDesktopIconWindowLevelKey))
        native_window.setLevel_(icon_level - 1)
        behavior = 0
        for name in (
        ...
        native_window.setIgnoresMouseEvents_(bool(mouse_through))
        ...
        _log().error("failed to configure macOS desktop-level NSWindow", exc_info=True)
        return False


def _set_mouse_through_macos(view: QWebEngineView, enable: bool) -> bool:
    try:
        native_window = _native_macos_window(view)
        if native_window is None:
            return False
        native_window.setIgnoresMouseEvents_(bool(enable))
        return True
    except Exception:
        _log().warning("failed to set macOS mouse-through=%s", enable, exc_info=True)
        return False
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run HTML wallpaper (macOS)")
...
def main() -> int:
    ...
    if screen is None:
        ...
        return 1
    # Span the complete virtual desktop. The NSWindow level keeps the content
    # below Finder icons/menu UI; using availableGeometry would leave gaps.
    ...
    def _platform_info() -> dict[str, Any]:
        ...
        current_geo = current_screen.virtualGeometry()
        return {
            "version": APP_VERSION,
            "platform": "macos",
            "screen": {
    ...
    view.showNormal()
    app.processEvents()
    if not _configure_macos_desktop_window(view, mouse_through=mouse_through):
        sys.stderr.write("Unable to place HTML renderer in the macOS desktop layer.\n")
        view.close()
        return 2
    ...
    def _poll_options() -> None:
        ...
        if "mouse_through" in data:
            ...
            if new_mt != state["mouse_through"]:
                ...
                _set_mouse_through_macos(view, enable=new_mt)

    def _tick_visibility() -> None:
        """Poll options and adjust page lifecycle based on focus state.

        v1.4.8: 旧版在 ``on_desktop is False`` 时也用 isActiveWindow 覆盖，
        会导致用户切到其他窗口时壁纸视图仍被 Qt 判为 "active"，auto-pause
        永远不触发。现在 False 表示明确检测到非桌面（非 Finder）前台，必须
        暂停；只有 None（检测失败）才回退到 view 状态。
        """
        _poll_options()
    ...
    # Increase the foreground poll interval to 3 seconds to reduce CPU wake-ups
    ...
    # Lengthen the keepalive interval to 30 seconds; this reduces CPU load
    # by sending fewer synthetic events when the page is unfrozen.
    ...
    def _on_screen_changed(_new_screen: Any = None) -> None:
        try:
            ...
            if sys.platform == "darwin":
                _configure_macos_desktop_window(view, mouse_through=state["mouse_through"])
