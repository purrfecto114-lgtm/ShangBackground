...
class ShangComboBox(QComboBox):
    ...
    def showPopup(self) -> None:  # noqa: N802 - Qt API
        ...
        # macOS 的 Qt 原生主题已经为弹出菜单绘制圆角和阴影。强制
        # WA_TranslucentBackground + FramelessWindowHint 会破坏原生主题，
        # 导致圆角不对称、阴影消失。QSS 的 border-radius 只控制菜单内部
        # 背景，窗口形状由系统/Qt 主题处理。
        try:
...
class ShangBackgroundWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        # v1.4.7: 移除应用内热键 (与打字冲突) —— _setup_app_shortcuts() 不再调用.
        # 全局热键 (Ctrl+Alt+...) 已足够.
...
"""
    ...
    def _build_ui(self):
        ...
        # 日志页面已移动到“全局设置”->“日志”中，因此此处不再添加日志标签页
    ...
    def _wallpaper_tab(self):
        ...
        # Hide horizontal scrollbars: long English translations or minimum widths
        # can otherwise cause the page to overflow and show a horizontal bar.
        ...
        # ---- HTML 交互式壁纸 ----
        # 与 Windows 端看齐：路径/URL、启动/停止/重启、自动暂停、GPU、鼠标穿透。
        # 底层实现仍遵守平台边界：Linux/macOS 没有 WorkerW，嵌入桌面层为 best-effort。
        ...
        self.html_edit = QLineEdit(core.config.get("html_file", "") or "")
        ...
        # Bug 19 fix: 更新说明，明确 macOS 上 HTML 壁纸的限制。
        mac_html_note = QLabel(t("macOS HTML 壁纸：创建无边框 Qt WebEngine 窗口置底显示，不会嵌入 Finder 桌面层。鼠标穿透通过 AppleScript 设置 ignoresMouseEvents，为全局开关。"))
        mac_html_note.setWordWrap(True)
        mac_html_note.setProperty("muted", True)
        html_layout.addWidget(mac_html_note, 4, 0, 1, 4)
        self.html_box = html_box
        # Bug 19 fix: 如果 html_wallpaper 模块不可用（依赖缺失），禁用所有 HTML 控件。
        _html_module_ok = getattr(core, "html_wallpaper", None) is not None
        if not _html_module_ok:
            for _w in (self.html_edit, self.html_browse_btn, self.html_start_btn,
                        self.html_stop_btn, self.html_restart_btn,
                        self.html_auto_pause_check, self.html_gpu_check,
                        self.html_mouse_through_check):
                _w.setEnabled(False)
        ...
        for btn in (btn_save, btn_restore_home):
            btn.setMinimumHeight(40)
        mh.addWidget(btn_save, 0, 0)
        mh.addWidget(btn_restore_home, 0, 1)
        action_tabs.addTab(maint_page, t("维护"))
        # Defer adding the quick actions box until after the preview box.  This
        # ensures quick actions sit below the preview and before any other panels.
        left.addStretch(1)
        ...
        # Wrap long history label onto multiple lines to avoid overflowing width.
        ...
        # Insert the quick actions panel after the preview.  Without this
        # reordering, the quick actions box can cause the overall width to exceed
        # the available space.
        right.addWidget(action_box)

        # Global hotkey enable switches. These intentionally reuse the ctx_*
        # widget names and config keys because the backend already uses these
        # flags to decide which actions should be registered globally.
        hotkey_box = QGroupBox(t("全局热键"))
        hotkey_layout = QVBoxLayout(hotkey_box)
        hotkey_layout.setContentsMargins(10, 18, 10, 10)
        ...
        for cb in (self.ctx_prev, self.ctx_next, self.ctx_random, self.ctx_jump):
            ...
            hotkey_layout.addWidget(cb)
        self._refresh_context_shortcut_labels()
        hotkey_hint = QLabel(t("启用后会注册对应动作的全局热键；可在“全局热键”设置页录制组合键。"))
        hotkey_hint.setWordWrap(True)
        hotkey_hint.setProperty("muted", True)
        hotkey_layout.addWidget(hotkey_hint)
        right.addWidget(hotkey_box)
...
"bing": t("同步必应"), "jump": t("跳转壁纸"), "about": t("关于"), "exit": t("退出"),
        ...
        # Cross-platform global hotkey recorder.  Unlike Windows, Linux/macOS
        # do not need single-letter context-menu mnemonics, so the settings
        # page only exposes a Record button, Clear button and the current value.
        shortcut_page = QWidget()
        shortcut_layout = QVBoxLayout(shortcut_page)
        shortcut_layout.setContentsMargins(0, 0, 0, 0)
        shortcut_layout.setSpacing(14)
        shortcut_box = QGroupBox(t("全局热键"))
        ...
        permission_hint = t("macOS 可能需要在系统设置中授予辅助功能权限。") if core.IS_MACOS else t("Linux 桌面环境可能需要允许键盘监听；Wayland 下可用性取决于桌面环境。")
        shortcut_hint = QLabel(
            t("点击“录制”后按下组合键；只有开启「启用全局热键」后才会注册为系统级热键。") + "\n" +
            t("当前 macOS/Linux 版本暂不支持 Windows 版的焦点防误触检测，请选择不易与常用软件冲突的组合键。") + "\n" +
            permission_hint
        )
        ...
        # v1.4.7: 移除应用内热键 UI (与打字冲突，由全局热键替代)
        self.app_sc_edits = {}
    ...
    def _reset_appearance_only(self) -> None:
        try:
            ...
            if reply != QMessageBox.StandardButton.Yes:
                return
            defaults = core.get_default_config()
            # v1.4.0: 确保重置全部外观相关键 (含 font_weight/font_size/performance_level)
    ...
    def _bing_tab(self):
        ...
        # Bug 21 fix: 增加最大宽度从 116 到 130，避免中文被截断。
    ...
    def eventFilter(self, obj, event):
        if getattr(self, "about_sprite_btn", None) is obj:
            if event.type() == QEvent.Type.Enter:
                ...
                self._fade_about_sprite_to(1)
        # v1.4.7: 移除 PgUp/PgDown 拦截块 —— 原本用于让应用内热键在 QScrollArea 上接管,
        # 但应用内热键已删除, 该拦截只会无谓吃掉 PgUp/PgDown.
        ...
        try:
            ...
            for widget, previous in reversed(previous_states):
                try:
                    ...
                    pass
            self._refreshing_from_config = False
            # v1.4.7: 应用内热键已移除, 不再调用 _refresh_app_shortcuts().
    ...
    # v1.4.7: _on_app_sc_edited / _reset_app_shortcut 已随应用内热键功能一并移除.
    ...
    def _flush_settings_dialog_unsaved_changes(self):
        ...
        覆盖范围：
        - Linux/macOS 有 app_sc_edits 字典时，遍历所有 QLineEdit 并调用 _on_app_sc_edited；
        - 字体路径编辑框（font_path_edit）如果存在且与 config 不一致则保存；
        - 其它即时保存的控件（如 DPI 滑块）不需要处理。
        """
        # 应用内快捷键编辑
        app_sc_edits = getattr(self, "app_sc_edits", None)
        if isinstance(app_sc_edits, dict):
            for action, edit in list(app_sc_edits.items()):
                try:
                    if not self._is_qobject_alive(edit):
                        continue
                    handler = getattr(self, "_on_app_sc_edited", None)
                    if callable(handler):
                        handler(action, edit)
                except Exception:
                    pass
    ...
    def _perform_exit_cleanup_once(self, *, restore_wallpaper: bool = True):
        ...
        # 停止 HTML 壁纸，防止退出时仍有残留进程
        try:
            core.stop_html_wallpaper()
        except Exception as exc:
            core.log(f"停止 HTML 壁纸失败: {exc}")
        # 退出时注销所有全局热键，确保 pynput listener 不残留。
        try:
            core.stop_global_hotkeys()
        except Exception as exc:
