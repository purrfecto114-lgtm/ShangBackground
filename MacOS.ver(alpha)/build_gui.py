...
def build_command(
    ...
    *,
    ...
    allowed_modes = {"standalone"} if EXPECTED_PLATFORM == "macos" else {"standalone", "onefile"}
    if mode not in allowed_modes:
        raise ValueError(f"Unsupported mode for {PLATFORM_LABEL}: {mode}")
...
class BuildLauncher:
    def __init__(self, root, tk, ttk, scrolledtext, messagebox):
        ...
        mode_values = ("standalone",) if EXPECTED_PLATFORM == "macos" else ("standalone", "onefile")
        ttk.Combobox(settings, textvariable=self.mode, values=mode_values, state="readonly", width=14).grid(row=0, column=3, sticky="w", pady=4)
