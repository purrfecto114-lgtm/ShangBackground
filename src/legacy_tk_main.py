...
# 全局常量
VERSION = "1.2.1"
TRANSITION_TEMP_DIR = os.path.join(BASE_DIR, "GuoduTemp")
...
def cleanup_transition_temp_files():
    """清理过渡动画临时文件目录中的所有文件。

    统一清理 TRANSITION_TEMP_DIR 中的临时帧文件，
    消除多处重复的临时文件清理代码。
    """
    if os.path.exists(TRANSITION_TEMP_DIR):
        for file in os.listdir(TRANSITION_TEMP_DIR):
            file_path = os.path.join(TRANSITION_TEMP_DIR, file)
            try:
...
def show_message(title, msg):
    """显示消息对话框，非 Windows 平台使用 messagebox，Windows 使用系统通知。"""
    if not IS_WINDOWS:
        try:
...
folder_entry = None
tray_icon_obj = None  # pystray.Icon 或 None，必须在模块级初始化，避免退出/提权时 NameError
...
def release_single_instance_mutex():
    """释放单实例互斥体，用于提权重启或退出前清理。"""
    global _instance_mutex_handle
    if _instance_mutex_handle is not None and IS_WINDOWS:
        try:
...
def restart_as_admin():
    ...
    try:
        ...
        # 传递命令行参数（跳过脚本名本身），去掉 --hide，提权重启后应显示主界面。
...
def _cleanup_tray_icon_on_exit():
    """模块级托盘图标清理函数，用于退出/提权重启前彻底清除托盘图标。

    1. 先尝试 pystray 的 visible=False + stop()
    2. 再用 Win32 API 强制刷新通知区域，让系统回收幽灵图标
    3. 最后短暂等待让 Windows 处理通知
    """
...
def send_command_to_hwnd(target_hwnd, command):
    ...
    try:
        ...
        result = ctypes.windll.user32.SendMessageW(HWND(int(target_hwnd)), UINT(WM_COPYDATA), WPARAM(0), lparam)
...
def load_config():
    ...
    default = {
        ...
        "ctx_personalize": False,  # 旧版“个性化设置/设置中心”右键菜单已移除
        "ctx_jump_to_wallpaper": False,
        "ctx_global_settings": False,  # 旧版设置中心右键菜单已移除
        "ctx_set_wallpaper": False,  # 图片文件“设为壁纸”右键菜单已移除
        "hotkey_previous": "U",
        "hotkey_next": "N",
        "hotkey_random": "3",
        "hotkey_jump": "V",
        ...
        "transition_effect": "frame",  # 仅保留帧动画
        ...
        try:
            ...
            if "tray_menu_items" in data and data["tray_menu_items"]:
                ...
                if isinstance(first_item, dict) and "action" in first_item:
                    ...
                    converted = True
            # 新版不再创建“设为壁纸”和“设置中心/个性化设置”右键菜单；
            # 如果旧配置曾用 ctx_global_settings 控制“跳转到壁纸”，迁移一次到 ctx_jump_to_wallpaper。
            ...
            if "app_theme" not in data:
                ...
                converted = True
            # 旧版高级转场已移除：旧配置统一迁移为帧动画，并清理无效键。
            if data.get("transition_effect") != "frame":
                data["transition_effect"] = "frame"
                converted = True
...
def save_config():
    ...
    with _config_lock:
        try:
            ...
            # 保证保存后的配置不再包含已删除的高级转场相关选项。
            config["transition_effect"] = "frame"
            with open(CONFIG_PATH, "w", encoding="utf-8") as f:
...
def report_usage():
    ...
    def _do_report():
        try:
            ...
            if not config.get("enable_usage_stats", False):
                log("使用统计上报未启用，已跳过")
                return
            bin_id = config.get("jsonbin_id") or os.environ.get("SHANGBACKGROUND_JSONBIN_ID", "")
            master_key = config.get("jsonbin_master_key") or os.environ.get("SHANGBACKGROUND_JSONBIN_MASTER_KEY", "")
            if not bin_id or not master_key or "马赛克" in bin_id or "马赛克" in master_key:
                log("使用统计上报未配置有效 JSONBin 凭据，已跳过")
                return
            url = f"https://api.jsonbin.io/v3/b/{bin_id}"
            headers = {"X-Master-Key": str(master_key), "Content-Type": "application/json; charset=utf-8"}
...
def push_wallpaper(path):
    """将壁纸路径推入历史记录，使用 deque 优化列表操作。"""
    if not path or not os.path.isfile(path):
        return
    hist = config.get("history", [])
    # 使用 deque 优化 O(n) 的 remove + insert 操作
    hist_dq = deque(hist, maxlen=50)
    if path in hist_dq:
        hist_dq.remove(path)
    hist_dq.appendleft(path)
    hist = list(hist_dq)
    config["history"] = hist
...
def start_slideshow():
    ...
    # 不在 slide_timer_lock 中执行系统壁纸设置；过渡动画/系统 API 都可能耗时。
    if target_to_apply:
...
def save_transition_state(state):
    ...
    try:
        ...
        os.makedirs(TRANSITION_TEMP_DIR, exist_ok=True)
        # 日志文件直接保存在根目录
...
def generate_transition_frames(current_img, target_img, fit_mode, frames=None):
    ...
    cleanup_transition_temp_files()
    if not os.path.exists(TRANSITION_TEMP_DIR):
        os.makedirs(TRANSITION_TEMP_DIR, exist_ok=True)
    ...
    def pre_resize_to_array(img_path, mode, size):
        """预调整图片大小并转为数组，使用公共函数 apply_image_fit_mode。"""
        with Image.open(img_path) as img:
            img_rgb = img.convert("RGB")
        result = apply_image_fit_mode(img_rgb, mode, size)
    ...
    if HAS_NUMPY:
        ...
        for i in range(frames):
            ...
            frame_path = os.path.join(TRANSITION_TEMP_DIR, f"frame_{i:04d}.bmp")
            blended_img.save(frame_path, "BMP")
            frame_paths.append(frame_path)
        return frame_paths
    else:
        img1 = pre_resize_to_array(current_img, fit_mode, target_size)
        img2 = pre_resize_to_array(target_img, fit_mode, target_size)
        frame_paths = []
        for i in range(frames):
            t = i / (frames - 1) if frames > 1 else 1.0
            blended_img = Image.blend(img1, img2, t)
            frame_path = os.path.join(TRANSITION_TEMP_DIR, f"frame_{i:04d}.bmp")
            blended_img.save(frame_path, "BMP")
            frame_paths.append(frame_path)
        return frame_paths
...
def update_preview(img_path):
    ...
    try:
        ...
        # 加载新图片并按适应模式处理
        with Image.open(img_path) as new_img_original:
            new_img_original = new_img_original.convert("RGB")
        ...
        def animate():
            ...
            if step <= frames:
                ...
                try:
                    ...
                    # 限制缓存帧数为5帧，防止长时间运行内存泄漏
                    if len(update_preview.animation_frames) > 5:
                        update_preview.animation_frames = update_preview.animation_frames[-5:]
...
def resize_image_for_preview(img, target_size, fit_mode):
    ...
    参数:
        img: PIL.Image 对象
        target_size: 目标预览区域尺寸 (width, height)
        fit_mode: 适应模式字符串

    返回:
        (处理后的PIL.Image, PhotoImage) 元组
    """
    target_w, target_h = target_size
    orig_w, orig_h = img.size
    ...
    # 使用公共函数 apply_image_fit_mode 处理适应模式
    img_fitted = apply_image_fit_mode(img, fit_mode, (preview_w, preview_h))
...
def show_overlay():
    ...
    之前会把 img/image.png 覆盖到壁纸缩略图上，里面的窗口/任务栏/文字会与真实图片混在一起。
    为了和新版 PySide6 GUI 保持一致，这里保留函数入口但不绘制任何遮罩。
    """
    return
...
def register_context(show_admin_prompt=False):
    ...
    if not is_windows_admin():
        msg = (
            "桌面右键菜单需要写入注册表 HKEY_CLASSES_ROOT，必须以管理员身份运行才能添加或移除。\n\n"
            "点击「确定」将以管理员身份重启应用并自动完成注册，\n"
            "点击「取消」则跳过注册，不影响主程序、托盘和壁纸切换功能。"
        )
        log("未以管理员权限启动，已跳过右键菜单注册/同步")
        if show_admin_prompt:
            # 使用 messagebox 弹窗，用户确认后自动提权重启
            result = messagebox.askokcancel("需要管理员权限", msg)
            if result:
                if restart_as_admin():
                    # 提权成功，退出当前实例
                    _do_exit(0)
                else:
                    messagebox.showerror("提权失败", "无法以管理员身份重启，请手动右键以管理员身份运行本程序。")
        return False
    try:
        ...
        def build_action_command(*args):
            ...
            if is_frozen():
                ...
                # 使用 sys.executable 而非 pythonw.exe，确保 PySide6 GUI 正确启动
        ...
        # 新版全局设置入口已移到 PySide6 首页齿轮按钮；桌面右键不再创建“个性化设置(&X)”。
        try:
        ...
        # 图片文件右键菜单“设为壁纸”和桌面右键“设置中心”均已移除；注册时只负责清理旧项。
        config["ctx_set_wallpaper"] = False
...
def toggle_ctx_global_settings():
    # 旧版兼容入口：设置中心右键菜单已移除，不再写入注册表。
    config["ctx_global_settings"] = False
    config["ctx_personalize"] = False
    save_config()
    register_context(show_admin_prompt=True)
...
def toggle_ctx_personalize():
    # 兼容旧调用：已合并到“全局设置”
    toggle_ctx_global_settings()


def toggle_ctx_jump():
    global ctx_jump_var
    value = bool(ctx_jump_var.get()) if ctx_jump_var is not None else bool(config.get("ctx_jump_to_wallpaper", False))
    config["ctx_jump_to_wallpaper"] = value
    config["ctx_global_settings"] = False
    config["ctx_personalize"] = False
    save_config()
    register_context(show_admin_prompt=True)
    ...
    def clear_selected_data(self):
        ...
        if set(to_clear) == set(all_keys):
            ...
            try:
                ...
                # 删除过渡动画临时文件夹
                if os.path.exists(TRANSITION_TEMP_DIR):
                    shutil.rmtree(TRANSITION_TEMP_DIR, ignore_errors=True)
                # 删除过渡日志
                if os.path.exists(TRANSITION_LOG_PATH):
                    os.remove(TRANSITION_LOG_PATH)
        ...
        for key in to_clear:
            if key == "slide_folder":
                ...
                config["ctx_next_wallpaper"] = False
                config["ctx_random_wallpaper"] = False
                config["ctx_jump_to_wallpaper"] = False
                config["ctx_global_settings"] = False
                config["ctx_personalize"] = False
                config["ctx_set_wallpaper"] = False
...
class TransitionSettingsDialog:
    ...
    def create_widgets(self):
        ...
        desc_label = tk.Label(
            left_frame,
            text="设置壁纸切换时的\n帧动画参数。",
            font=(FONT_FAMILY, 8), bg="#9BFFF2", fg="#7f8c8d", justify="center"
        )
        desc_label.pack(pady=(10, 0))
        ...
        tk.Label(effect_row, text="转场原理", font=(FONT_FAMILY, 10), fg="#34495e", bg="#ffffff", width=8, anchor="w").pack(side="left")
        tk.Label(effect_row, text="帧动画", font=(FONT_FAMILY, 10), fg="#34495e", bg="#ffffff").pack(side="left", padx=5)

        duration_row = tk.Frame(right_frame, bg="#ffffff")
        duration_row.pack(fill="x", pady=(0, 20))
        tk.Label(duration_row, text="持续时间", font=(FONT_FAMILY, 10), fg="#34495e", bg="#ffffff", width=8, anchor="w").pack(side="left")

        self.duration_var = tk.DoubleVar(value=self.transition_duration)

        def on_duration_spinbox_change(*args):
            duration_scale.set(self.duration_var.get())
        ...
        frame_row = tk.Frame(right_frame, bg="#ffffff")
        frame_row.pack(fill="x", pady=(0, 20))
        tk.Label(frame_row, text="帧数", font=(FONT_FAMILY, 10), fg="#34495e", bg="#ffffff", width=8, anchor="w").pack(side="left")

        self.frame_var = tk.IntVar(value=self.transition_frames)

        def on_frame_spinbox_change(*args):
            frame_scale.set(self.frame_var.get())

        frame_spinbox = ttk.Spinbox(frame_row, from_=4, to=30, textvariable=self.frame_var, width=8,
                                    font=(FONT_FAMILY, 10), command=on_frame_spinbox_change)
        frame_spinbox.pack(side="left", padx=(0, 10))
        tk.Label(frame_row, text="帧", font=(FONT_FAMILY, 10), fg="#7f8c8d", bg="#ffffff").pack(side="left")

        frame_scale = ttk.Scale(frame_row, from_=4, to=30, orient="horizontal", length=180)
...
def handle_command_line():
    ...
    parser.add_argument('--legacy-tk', action='store_true', help='兼容新版入口参数')
    ...
    if args.previous:
        ...
        log("收到 --jump-to-wallpaper 命令，以 PySide6 侧边栏响应")
        try:
            folder = config.get("slide_folder", "")
            current = config.get("current_wallpaper", "")
            if not folder or not os.path.isdir(folder):
            ...
            # WallpaperSidebar 已迁移到 PySide6；在任何情况下都先建立 QApplication
            try:
                from PySide6.QtWidgets import QApplication
                from PySide6.QtGui import QIcon
                from wallpaper_sidebar import WallpaperSidebar
            except ImportError as ie:
                log(f"PySide6 / wallpaper_sidebar 不可用: {ie}")
                show_message("错误", f"无法打开侧边栏（PySide6 不可用）:\n{ie}")
                sys.exit(0)

            _qt_app = QApplication.instance() or QApplication(sys.argv)
            _icon = os.path.join(BASE_DIR, "img", "LOGO.ico")
            if os.path.exists(_icon):
                _qt_app.setWindowIcon(QIcon(_icon))
            ...
            sidebar = WallpaperSidebar(
                None, folder, current, None,
                show_message=show_message,
                switch_wallpaper=_switch,
            )
            sidebar.closed.connect(_qt_app.quit)   # 侧边栏关闭 → 退出事件循环
            _qt_app.exec()
            sys.exit(0)
...
def main():
    ...
    global ctx_prev_var, ctx_next_var, ctx_random_var, ctx_personalize_var, ctx_jump_var, ctx_global_settings_var, ctx_file_wallpaper_var, wallpaper_monitor_running, wallpaper_monitor_last
    ...
    # ==================== 最优先：单实例检测 ====================
    # 必须在任何 UI 创建之前执行，避免重复创建窗口导致图标失效
    # 先快速解析命令行参数，判断是否为"动作启动"（如 --show），决定是否显示提示
    _early_parser = argparse.ArgumentParser(add_help=False)
    _early_parser.add_argument('--legacy-tk', action='store_true')
    _early_parser.add_argument('--previous', action='store_true')
    _early_parser.add_argument('--next', action='store_true')
    _early_parser.add_argument('--random', action='store_true')
    _early_parser.add_argument('--show', action='store_true')
    _early_parser.add_argument('--hide', action='store_true')
    _early_parser.add_argument('--jump-to-wallpaper', action='store_true')
    _early_parser.add_argument('--set-wallpaper', type=str)
    _early_args, _ = _early_parser.parse_known_args()
    _is_action_launch = (_early_args.previous or _early_args.next or _early_args.random or _early_args.show)

    allow_legacy_secondary = os.environ.get("SHANGBACKGROUND_LEGACY_SECONDARY") == "1"
    if IS_WINDOWS and not allow_legacy_secondary:
        if not acquire_single_instance_mutex():
            log("检测到已有实例（互斥体），打开现有主界面并退出本次启动")
            # 如果是 --show 等动作启动，不显示"不要重复运行"提示
            activate_existing_instance(show_notice=not _is_action_launch)
            sys.exit(0)
        existing = find_existing_main_window(timeout=0.3)
        if existing:
            log("检测到已有主窗口，打开现有主界面并退出本次启动")
            release_single_instance_mutex()  # 释放刚获取的互斥体（因为要退出了）
            activate_existing_instance(show_notice=not _is_action_launch)
            sys.exit(0)
    elif allow_legacy_secondary:
        log("旧版界面以从属设置窗口模式启动：跳过互斥体抢占和主窗口唤醒。")
    ...
    # 命令行参数处理完毕后，再次确认无已有实例（防止命令行处理期间有新实例启动）
    # 注意：互斥体已在最前面获取，此处仅做窗口级别的补充检查
    ...
    args_for_check.add_argument('--legacy-tk', action='store_true')
    ...
    # 补充检查：在消息窗口创建后再次确认（此时 hwnd 已有值，可以排除自身）
    # 此检查主要用于兼容互斥体因权限差异无法跨会话检测的极端情况
    if IS_WINDOWS:
        existing_hwnd = ctypes.windll.user32.FindWindowW(WND_CLASS_NAME, None)
        if existing_hwnd and existing_hwnd != hwnd:
            log("补充检查：检测到已有主进程，打开已有主界面并退出本实例")
            if pending_action:
    ...
    def load_title_image_after_window_ready():
        ...
        try:
            ...
            with Image.open(title_img_path) as original_img:
    ...
    # ========== 托盘图标相关函数 ==========
    # 注意：tray_icon_obj 使用全局变量（已在文件顶部声明），不要在此处重新赋值 None
    # 否则会创建局部变量遮蔽全局变量，导致模块级清理函数无法访问实际对象
    ...
    def open_tray_settings():
        ...
        click_action_values = list(click_action_options.values())
        click_action_reverse = {v: k for k, v in click_action_options.items()}

        current_click_action = config.get("tray_click_action", "next")
        current_click_label = click_action_options.get(current_click_action, "下一张壁纸")

        click_combo = ttk.Combobox(combo_container, values=click_action_values,
                                   state="readonly", width=18, font=(FONT_FAMILY, 9))
        click_combo.set(current_click_label)
        click_combo.pack(side="left")

        tip_label = tk.Label(combo_container, text="<-- 选择单击托盘图标时的行为",
                             font=(FONT_FAMILY, 8), fg="#7f8c8d", bg="#ffffff")
        ...
        def save_settings():
            ...
            # 保存单击托盘图标行为
            selected_label = click_combo.get()
            selected_action = click_action_reverse.get(selected_label, "next")
            config["tray_click_action"] = selected_action
    ...
    def destroy_tray_icon():
        ...
        if tray_icon_obj is not None:
            ...
            try:
                ...
                log(f"停止托盘图标失败: {e}")
            try:
                time.sleep(0.08)  # 稍微延长等待，确保 Windows 通知区完成刷新
            except Exception:
                pass
    ...
    def update_ctx_checkbutton_texts():
        global chk_prev, chk_next, chk_random, chk_jump
        # 获取各菜单的快捷键
    ...
    ctx_next_var = tk.BooleanVar(value=config.get("ctx_next_wallpaper", False))
    ...
    ctx_global_settings_var = tk.BooleanVar(value=config.get("ctx_global_settings", False))
    ctx_jump_var = ctx_global_settings_var
    ctx_personalize_var = tk.BooleanVar(value=False)
    global chk_jump, chk_personalize
    chk_jump = ttk.Checkbutton(extra_frame, text="", variable=ctx_global_settings_var, command=toggle_ctx_global_settings)
    chk_jump.pack(anchor="w", pady=0)
    chk_personalize = chk_jump
    ...
    if is_first_time:
        # 右键菜单默认保持全不选；只保留随机顺序、后台运行、托盘图标等非注册表建议项。
        ...
        config["ctx_jump_to_wallpaper"] = False
        save_config()
        log("首次使用：右键菜单默认全不选；已保留随机顺序、后台运行、托盘图标建议项")
        ...
        if not config.get("auto_start_prompt_shown", False):
            ...
            if os.path.exists(hello_img_path):
                try:
                    with Image.open(hello_img_path) as hello_img:
                        # 缩放到0.12倍（约96x96）
                        scale = 0.12
...
if __name__ == "__main__":
    # 注册 atexit 清理函数，确保异常退出时也能释放互斥体和清理托盘图标
    import atexit
    atexit.register(release_single_instance_mutex)
    atexit.register(_cleanup_tray_icon_on_exit)
    main()