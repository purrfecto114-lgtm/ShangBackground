...
from app.http_utils import read_json_object, validate_final_url
...
class BingDownloader:
        ...
        for api in self.API_URLS:
            try:
                with self._open(api, params=params, timeout=20) as response:
                    validate_final_url(response, self._is_allowed_bing_url, fallback_url=api)
                    data = read_json_object(response, MAX_METADATA_JSON_BYTES)
    ...
    def download_wallpaper(self, info: WallpaperInfo, resolution: str | None = None) -> Optional[str]:
        # Synchronize cache discovery and the .part -> final transaction so two
        # GUI workers cannot race on the same image file.
        with self._cache_lock:
            return self._download_wallpaper_locked(info, resolution)

    def _download_wallpaper_locked(self, info: WallpaperInfo, resolution: str | None = None) -> Optional[str]:
            ...
            try:
                ...
                try:
                    with self._open(url, timeout=30) as response:
                        final_url = validate_final_url(response, self._is_allowed_bing_url, fallback_url=url)
    ...
    @staticmethod
    def _cache_sort_key(path: Path) -> tuple[float, str]:
        try:
