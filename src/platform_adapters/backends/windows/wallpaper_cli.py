...
_DEBUG_DESTINATION_UNSET = object()
_debug_destination_cache = _DEBUG_DESTINATION_UNSET

def log_debug(msg):
    """Emit diagnostics only when explicitly enabled, without delaying shell menus."""
    global _debug_destination_cache
    if _debug_destination_cache is _DEBUG_DESTINATION_UNSET:
        _debug_destination_cache = _debug_log_destination()
    destination = str(_debug_destination_cache or "")
    if _env_flag("SHANGBACKGROUND_RIGHTCLICK_DEBUG"):
        print(msg)
    if not destination:
        return
    try:
...
def main():
    ...
    if len(images) == 1:
        ...
        try:
            ...
            # 与 Linux/MacOS 端 wallpaper_cli.py 行为对齐：显示错误对话框后直接 return。
