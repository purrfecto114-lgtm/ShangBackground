"""System-native HTML wallpaper process adapter for Windows.
...
``native_html_runner.py`` delegates WorkerW placement and mouse-through
behavior to ``backends/windows/native_webview_desktop.py``. Explorer/WorkerW behavior
remains best-effort and must be validated on supported Windows versions.
"""
...
PROCESS_KIND = "html-wallpaper-windows"
...
def start_html_wallpaper(path: str) -> Tuple[bool, str]:
    ...
    * ``auto_pause``  (default True)  — freeze the page only when every display is visually covered.
    """
    ...
    # Determine the child command. In a frozen build there is no loose runner
    # script beside the executable, so re-enter the bundled EXE through the
    # native HTML dispatch flag.
...
def get_last_path() -> str:
    ...
    Used by the GUI to restart the wallpaper (e.g. after changing startup-only options) when
...
def restart_html_wallpaper(path: str | None = None) -> Tuple[bool, str]:
    ...
    Used by the GUI when a setting that can't be hot-reloaded (a startup-only option
