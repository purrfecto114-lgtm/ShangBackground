"""System-native HTML wallpaper process adapter for Linux.

The adapter validates local/remote sources, owns the system-native HTML child
process, persists verified process identity, and forwards hot options. The
shared ``native_html_runner.py`` delegates X11 desktop-layer placement and
mouse-through behavior to ``backends/linux/native_webview_desktop.py``. Generic
Wayland sessions are rejected unless an explicit unsafe test override is set.
"""
...
PROCESS_KIND = "html-wallpaper-linux"
...
# stop_html_wallpaper to avoid orphaned renderer processes that can
...
def start_html_wallpaper(path: str) -> Tuple[bool, str]:
    ...
    Reads ``auto_pause`` from the runtime options file
    ...
    if (
        ...
        return False, (
            "当前 HTML 后端无法在通用 Wayland/XDG Shell 下可靠定位到桌面背景层。"
