...
using a small grid on each display.  If any display has more than 10% desktop
    ...
    *,
    ...
    try:
        ...
        def _enum(hwnd, _lparam):
            try:
                if int(hwnd) in excluded_window_ids:
                    return True
...
def _macos_covering_rects(
    ...
    *,
    ...
    try:
        ...
        for info in infos:
            try:
                window_number = int(info.get(Quartz.kCGWindowNumber, 0) or 0)
                if window_number in excluded_window_ids:
                    continue
    ...
    try:
        ...
        try:
            ...
            for window in windows:
                try:
                    if int(window) in excluded_window_ids:
                        continue
...
_last_probe_key: tuple[int, ...] = ()


def _normalize_window_ids(values) -> frozenset[int]:
    result: set[int] = set()
...
def desktop_is_visible(
    *,
    force: bool = False,
    excluded_window_ids: set[int] | frozenset[int] | tuple[int, ...] = (),
) -> bool | None:
    """Return whether any connected display still shows the desktop.

    ``False`` means every display is at least 90% covered and the HTML page may
    ...
    """
    global _last_probe_at, _last_probe_key, _last_probe_result
    ...
    if not screens:
        ...
        windows = _windows_covering_rects(screens, excluded_window_ids=excluded)
