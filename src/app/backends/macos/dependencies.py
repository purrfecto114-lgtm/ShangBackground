...
def _try_pyside_prompt(missing, packages, parent=None) -> bool | None:
    """Show a PySide6 QMessageBox with the install command and two buttons:
    'Copy command', 'Open Terminal', and optional 'Do not remind'.

    Returns ``None`` only when Qt is unavailable; otherwise returns whether the
    ...
    if not any(dep.get("required") for dep in missing):
        suppress_btn = msg.addButton(t("不再提醒"), QMessageBox.ButtonRole.ActionRole)
...
def prompt_install_dependencies(_notifier, availability, parent=None, prefer_pyside=None):
    ...
    result = _try_pyside_prompt(missing, packages, parent=parent)
