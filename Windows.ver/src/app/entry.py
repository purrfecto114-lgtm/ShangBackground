...
def _read_log_enabled_from_config() -> bool:
    ...
    try:
        import json
        # ``core.CONFIG_PATH`` is set at module-import time, so it's available
        # even before ``core.load_config()`` is called.
...
def main() -> int:
    ...
    #
    # IMPORTANT: ``force=True`` is required because ``core.engine`` import
    # (line 8 above) triggers ``load_config()`` at module level (engine.py
    # line 1581), which calls ``log()`` → ``LegacyAdapter.log()`` →
    # ``configure_logging()`` with DEFAULTS (files_enabled=False).  That
    # early call sets ``_CONFIGURED=True``, so without ``force=True`` this
    # call would be a NO-OP and the user's ``log_enabled=True`` setting
    # would never take effect.  ``force=True`` reconfigures with the correct
    ...
    if not sys.platform.startswith("win"):
        ...
        print("WARNING: This version of ShangBackground is for Windows only.", file=sys.stderr)
        ...
        _result = QMessageBox.question(
            ...
            str(t("当前版本仅适用于 Windows 系统。")) + "\n\n" +
            str(t("检测到当前系统非 Windows，继续运行可能导致异常。")) + "\n\n" +
            str(t("是否仍要继续运行？")),
    ...
    if core.IS_WINDOWS:
        if _is_already_running():
            if direct_action_launch:
                core.log("检测到已有实例，转发右键菜单/命令行动作并退出")
                if not _dispatch_action_to_existing_instance(args):
    ...
    # ``core.config`` is now fully loaded (by ``load_config()`` at engine.py
    # import time).  ``configure_logging(force=True)`` above already set the
    ...
    def _post_show_runtime_startup():
        ...
        # 初始化全局热键（Ctrl+Alt+N 等组合键，任何应用前台时都能触发壁纸切换）
        if core.IS_WINDOWS:
