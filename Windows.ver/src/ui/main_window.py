...
from PySide6.QtGui import QAction, QColor, QDesktopServices, QIcon, QImageReader, QPainter, QPalette, QPixmap
...
class ShangComboBox(QComboBox):
    """Compact combo box that uses one guarded QMenu popup.

    The native QComboBox popup can grow to the viewport width and may show
    top/bottom scroller arrows when the style decides the popup is clipped.
    A menu-based popup is smaller, avoids the double rounded-border artifact,
    and keeps behaviour deterministic across Windows/Linux/macOS builds.
    """
    ...
    def _clear_popup_menu_ref(self, menu) -> None:
        try:
            ...
            pass
        # Reset the base QComboBox popup state as recommended for custom popups.
        try:
    ...
    def showPopup(self) -> None:  # noqa: N802 - Qt API
        ...
        if self._menu_alive(active):
            ...
            ShangComboBox._active_menu = None
            if same_owner:
                # Clicking the same combo while its menu is open now toggles it closed
                # instead of stacking another menu on top of the first one.
                self._popup_menu = None
                return
        ...
        try:
            ...
            pass
        # 注意：不要在 QMenu 上设置 WA_TranslucentBackground。
        # 之前为了"让 QSS border-radius 生效"加了 WA_TranslucentBackground +
        # FramelessWindowHint + NoDropShadowWindowHint，但在 Windows 上反而会
        # 破坏原生菜单的圆角窗口形状（系统不再用 DWM 绘制圆角），导致左右圆角
        # 不对称、阴影消失。QSS 的 border-radius 只控制菜单内部背景的圆角，
        # 窗口本身的圆角由系统原生处理，保留原生行为即可。
        try:
        ...
        for row in range(self.count()):
            ...
            try:
                enabled = bool(self.model().index(row, self.modelColumn(), self.rootModelIndex()).flags() & Qt.ItemFlag.ItemIsEnabled)
            except Exception:
                enabled = True
        ...
        try:
            # 计算菜单高度时，必须用 QSS 实际渲染的 item 高度。当前 QSS 规则是
            # ``min-height: 28px; padding: 4px 12px;`` → 单项总高约 36px (28 + 4*2)。
            # 用 36px 而不是 fontMetrics().height() + 16（仅 ~30px），否则 6 项模式
            # 会被截断，最后一项 HTML 被裁掉不可见。
            item_h = max(36, menu.fontMetrics().height() + 22)
            visible_rows = max(1, min(self.count(), 12))
            target_h = visible_rows * item_h + 12  # 上下 padding 各 ~6px
            if self.count() <= 12:
                menu.setMinimumHeight(target_h)
            else:
                menu.setMaximumHeight(target_h)
        except Exception:
            pass
        menu.aboutToHide.connect(lambda menu=menu: self._clear_popup_menu_ref(menu))
        self._popup_menu = menu
        ShangComboBox._active_menu = menu
        # Bug 16 fix: 旧版直接用 bottomLeft() 作为弹出原点，当 combo 在窗口右边缘时
        # 菜单会超出屏幕右边界被裁剪（不对称）。改为根据屏幕可用区域计算最佳位置：
        # 优先左对齐，若右溢出则右对齐；若底部溢出则向上弹出。
        ...
        try:
            ...
            if screen is not None:
                ...
                # 水平：默认左对齐；若右溢出则右对齐到 combo 右边缘
                if pos.x() + menu_w > avail.right():
                    pos.setX(self.mapToGlobal(self.rect().topRight()).x() - menu_w)
                # 垂直：默认在下方；若底部溢出则弹到上方
                if pos.y() + menu_h > avail.bottom():
                    pos.setY(self.mapToGlobal(self.rect().topLeft()).y() - menu_h)
                # 保底：不要超出屏幕左/上边界
                if pos.x() < avail.left():
                    pos.setX(avail.left())
                if pos.y() < avail.top():
...
class CompactSpinBox(QSpinBox):
    ...
    QGridLayout may otherwise stretch QSpinBox cells when the surrounding
    form has spare horizontal space.  This subclass keeps Bing count controls
    visually compact because the value is only one to three digits.
    """
    ...
    def __init__(self, width: int = 64, *args, **kwargs):
        ...
        self.setMinimumWidth(self._compact_width)
        self.setMaximumWidth(self._compact_width)
...
class ShangBackgroundWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        # v1.4.7: 移除应用内热键注册 (_setup_app_shortcuts) —— 与打字冲突 + 焦点锁死.
        # 全局热键 (Ctrl+Alt+...) 已足够.
    ...
    def _svg_current_color(self) -> str:
        ...
        try:
            ...
            if dark:
                if brightness >= 230:
                    # Very light accent → button bg is darkened → icon should be light.
                    return "#e6e6f0"
    ...
    def _deferred_gui_startup(self):
        ...
        if level == "power_saver":
            # 省电模式: 最长延迟, 最低资源占用
            _d_preview, _d_bing, _d_native, _d_tray, _d_autostart, _d_bingtask, _d_status = 400, 1200, 700, 950, 2200, 2800, 1700
        elif level == "performance":
            # 性能模式: 中等延迟 (原 performance_mode=True)
            _d_preview, _d_bing, _d_native, _d_tray, _d_autostart, _d_bingtask, _d_status = 260, 900, 520, 720, 1700, 2200, 1300
        else:
            # 平衡模式 (默认): 最短延迟 (原 performance_mode=False)
            _d_preview, _d_bing, _d_native, _d_tray, _d_autostart, _d_bingtask, _d_status = 100, 320, 260, 420, 900, 1250, 950
    ...
    # v1.4.7: 移除 changeEvent / _sync_hotkey_focus_state ——
    # 这两个方法是 v1.4.6 为"应用内热键让位全局热键"加的, 现在应用内热键已移除,
    # 全局热键始终注册, 不需要窗口激活/失焦时切换. 之前 _sync_hotkey_focus_state
    # 会在窗口激活时 _suspend_global_hotkeys_temporarily(), 如果失焦事件没正常触发,
...
QToolTip { background-color: __BG_INPUT__; color: __FG_PRIMARY__; border: 1px solid __BORDER__; border-radius: 6px; padding: 6px 10px; font-size: 12px; }
...
"""
    ...
    def _refresh_styled_widgets(self):
        ...
        try:
            ...
            pass
        try:
            # Iterate through all widgets and fully refresh their style.  Calling
            # unpolish/polish alone isn't always enough when the style changes the
            # widget's box model (padding, margins or size).  See Qt forum
            # discussion where a style change event is also needed to recompute
            # geometry.
            for widget in app.allWidgets():
                # Restore proper button symbols for spin boxes after theme
                # changes.  Without this call, some platforms may show only
                # plain arrows or hide the up/down icons entirely.
                if isinstance(widget, (QSpinBox, QDoubleSpinBox)):
                    try:
                        ...
                        pass
                try:
                    # Remove the current style so that the next polish picks up
                    # the updated palette and stylesheet.
                    widget.style().unpolish(widget)
                    widget.style().polish(widget)
                    # Send a StyleChange event to force a recomputation of
                    # layout metrics such as padding and margins.  Without
                    # this the widget may not recalculate its geometry on
                    # certain Qt versions.
                    event = QEvent(QEvent.Type.StyleChange)
                    QApplication.sendEvent(widget, event)
                    # Update and refresh geometry so the new style takes effect.
                    widget.update()
                    widget.updateGeometry()
                except Exception:
                    # Some widgets may be deleted or not support polishing;
                    # ignore failures to avoid interrupting the refresh loop.
                    continue
        except Exception:
            # Protect against unexpected errors from app.allWidgets() so the
    ...
    def _build_ui(self):
        ...
        # 修复 (v1.4.6): 把"展开/收起操作详情"按钮放进固定宽度的容器,
        # 避免 status_label 文本长度变化(如翻译切换)时按钮位置漂移.
        ...
        # Bug 13 fix: 启用自动换行 + stretch=1，避免英文/长翻译下撑挤“请求终止”按钮。
        op_hint.setWordWrap(True)
        op_layout.addWidget(op_hint, 1)
        ...
        # 日志页面移动到“全局设置”->“日志”中，因此这里不再添加日志标签页
    ...
    def _wallpaper_tab(self):
        ...
        # Never show horizontal scroll bars on the home page.  Some long labels or
        # minimum sizes can otherwise cause the body widget to exceed the view width,
        # resulting in an unexpected horizontal scrollbar in English mode.
        ...
        #
        # Bug 1 fix: 旧版用 5:3 比例 (62.5/37.5) + right_panel setMaximumWidth(640)
        # 导致左列过宽、右列狭窄。改为 3:2 比例 (60/40) + left_panel setMaximumWidth(720)
        # 让两列在宽窗口下都保持合理宽度，避免大块空带。
        left_panel = QWidget()
        right_panel = QWidget()
        ...
        # 让 combo/line-edit 填满列宽，避免控件挤在左上角。
        ...
        # Increase the maximum width of the mode selector so longer mode names
        # (e.g. "幻灯片放映") are not truncated.  The previous value (236) was
        # insufficient for some translations.  A larger width still fits within
        # the form layout while improving readability.
        ...
        # Bug 21 fix: 该按钮文字较长，需要足够宽度避免被截断。
        ...
        self.video_edit.setPlaceholderText(t("选择 mp4/mov/mkv/webm 视频作为桌面背景"))
        ...
        # 引导提示：当未选视频文件时显示，提醒用户先选视频
        self.video_volume_hint_label = QLabel(t("请先选择视频文件后再调整音量"))
        self.video_volume_hint_label.setProperty("muted", True)
        self.video_volume_hint_label.setProperty("dialogNote", True)
        ...
        # 初始显示：未选视频文件时显示引导，否则隐藏
        self.video_volume_hint_label.setVisible(not _has_video_file)
        video_layout.addWidget(QLabel(t("视频文件")), 0, 0)
        ...
        video_layout.addWidget(QLabel(t("音量")), 2, 0)
        video_layout.addWidget(video_volume_row, 2, 1, 1, 3)
        video_layout.addWidget(QLabel(t("失焦处理")), 3, 0)
        video_layout.addWidget(self.video_focus_behavior_combo, 3, 1, 1, 3)
        video_layout.addWidget(self.video_volume_hint_label, 4, 0, 1, 4)
        self.video_box = video_box
        left.addWidget(video_box)

        # ---- HTML 交互式壁纸 ----
        # 该区块曾因重构被误删，此处恢复：支持本地 HTML 文件 / 远程 URL，
        # 由独立进程 QWebEngineView 渲染并定位到桌面底层。
        # 新增“桌面失焦自动暂停”选项：当用户不在桌面（有其他窗口在前台）时，
        # WebEngine 会进入 Frozen 状态释放 CPU/GPU，回到桌面再恢复渲染。
        # 新增“启用 GPU 加速”选项：默认开启；关闭后强制软件渲染，可解决部分
        # 显卡驱动 / 远程桌面下的花屏或崩溃问题。该选项无法热更新，切换后会
        # 自动重启 HTML 壁纸子进程以使新设置生效。
        # 桌面图标覆盖问题已修复：采用 SetParent(WorkerW) 嵌入技术，把壁纸
        # 窗口作为 WorkerW 的子窗口渲染到桌面背景与桌面图标之间，图标可正常
        # 显示和点击。
        html_box = QGroupBox(t("HTML 壁纸"))
        html_layout = QGridLayout(html_box)
        html_layout.setHorizontalSpacing(10)
        html_layout.setVerticalSpacing(8)
        self.html_edit = QLineEdit()
        self.html_edit.setPlaceholderText(t("选择本地 HTML 文件或输入 http(s) 网址"))
        self.html_browse_btn = QPushButton(t("选择 HTML"))
        self.html_browse_btn.setProperty("secondary", True)
        self.html_start_btn = QPushButton(t("启动 HTML"))
        self.html_stop_btn = QPushButton(t("停止 HTML"))
        self.html_restart_btn = QPushButton(t("重启 HTML"))
        self.html_restart_btn.setToolTip(t("停止并重新启动当前 HTML 壁纸（用于应用 GPU 切换等无法热更新的设置）"))
        self.html_auto_pause_check = QCheckBox(t("桌面被遮挡时自动暂停（节省 CPU/GPU）"))
        self.html_auto_pause_check.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Fixed)
        self.html_auto_pause_check.setToolTip(
            t("当所有显示器几乎被窗口遮挡时，自动冻结 HTML 壁纸的渲染与脚本；") + "\n" +
            t("任一显示器仍能看到桌面时继续运行；无法可靠检测的平台不会自动暂停。")
        )
        self.html_gpu_check = QCheckBox(t("启用 GPU 加速"))
        self.html_gpu_check.setToolTip(
            t("开启后由显卡硬件加速渲染，画面更流畅、CPU 占用更低；") + "\n" +
            t("关闭后改用软件渲染，可解决部分显卡驱动、远程桌面或虚拟机下的花屏/崩溃问题。") + "\n" +
            t("该选项无法热更新，切换后会自动重启 HTML 壁纸。")
        )
        # 鼠标穿透开关：默认开启，让桌面图标可点击；关闭后壁纸可接收鼠标事件，
        # 适用于需要点击交互的 HTML 壁纸（按钮、链接、控件等）。
        # 通过 WS_EX_TRANSPARENT 实现，可热更新（无需重启壁纸）。
        self.html_mouse_through_check = QCheckBox(t("鼠标穿透（让桌面图标可点击）"))
        self.html_mouse_through_check.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Fixed)
        self.html_mouse_through_check.setToolTip(
            t("开启后鼠标点击会穿透壁纸直达桌面图标（默认）；") + "\n" +
            t("关闭后壁纸可接收鼠标事件，适用于需要点击交互的 HTML 壁纸。") + "\n" +
            t("该选项可热更新，切换后立即生效，无需重启壁纸。")
        )
        self.html_browse_btn.clicked.connect(self.choose_html_file)
        self.html_start_btn.clicked.connect(self.start_html_wallpaper_from_gui)
        self.html_stop_btn.clicked.connect(lambda: self.run_core(core.stop_html_wallpaper))
        self.html_restart_btn.clicked.connect(self.restart_html_wallpaper_from_gui)
        # 复选框状态变化时即时持久化，下次启动脚本会读取
        self.html_auto_pause_check.toggled.connect(self.on_html_auto_pause_changed)
        self.html_gpu_check.toggled.connect(self.on_html_gpu_changed)
        self.html_mouse_through_check.toggled.connect(self.on_html_mouse_through_changed)
        for _btn in (self.html_browse_btn, self.html_start_btn, self.html_stop_btn, self.html_restart_btn):
            _btn.setMinimumHeight(34)
        html_layout.addWidget(QLabel(t("HTML 路径")), 0, 0)
        html_layout.addWidget(self.html_edit, 0, 1, 1, 2)
        html_layout.addWidget(self.html_browse_btn, 0, 3)
        html_layout.addWidget(self.html_start_btn, 1, 1)
        html_layout.addWidget(self.html_stop_btn, 1, 2)
        html_layout.addWidget(self.html_restart_btn, 1, 3)
        html_layout.addWidget(self.html_auto_pause_check, 2, 1, 1, 3)
        html_layout.addWidget(self.html_gpu_check, 3, 1)
        html_layout.addWidget(self.html_mouse_through_check, 3, 2, 1, 2)
        ...
        btn_admin_home = QPushButton(t("重启程序") if core.is_windows_admin() else t("管理员重启"))
        btn_admin_home.setProperty("secondary", True)
        btn_admin_home.setToolTip(t("已是管理员权限时执行普通重启；否则请求管理员权限重启。"))
        self._set_button_svg_icon(btn_admin_home, "restart.svg", size=20)
        ...
        # Defer adding the quick actions box until after the preview box.  Placing
        # it between the preview and the context menu avoids excess horizontal width
        # and keeps the layout consistent when translations are long.
        # The call to add `action_box` happens later after the preview is inserted.
        left.addStretch(1)

        preview_box = QGroupBox(t("当前壁纸"))
        # Bug 1 fix: 放宽 preview_box 最小宽度（300→280）并改为 Expanding/Expanding
        # 让预览能填满右侧可用空间。旧版用 Expanding/Maximum 导致预览下方留空带。
        ...
        # Bug 1 fix: 跳转按钮独立一行，让路径输入框获得全部宽度，避免被截断。
        ...
        # Bug 21 fix: 增加最大宽度从 140 到 160，避免中文被截断。
        ...
        # Wrap the long history label to prevent the layout from expanding horizontally
        ...
        fav_title = QLabel(t("收藏夹（单击应用，双击打开位置，右键移除收藏）"))
        ...
        # Now insert the quick actions panel after the preview.  This ensures
        # quick actions sit between the preview and the context menu.
        right.addWidget(action_box)

        if core.IS_WINDOWS:
            ctx_box = QGroupBox(t("右键菜单"))
            ctx_box.setMinimumWidth(360)
        ...
        # 制造可见的空带。about_row 现在直接跟在 ctx_box 之后，紧凑布局。
    ...
    def _settings_tab_full(self):
        ...
        # 向后兼容: 保留 perf_mode_check 属性为 None (旧代码引用时不报错)
        self.perf_mode_check = None
...
"bing": t("同步必应"), "jump": t("跳转壁纸"), "about": t("关于"), "exit": t("退出"),
        ...
        if core.IS_WINDOWS:
            ...
            shortcut_box = QGroupBox(t("全局热键设置"))
            ...
            # v1.4.7: 这些快捷键现在 ONLY 作为全局热键注册 (Ctrl+Alt+... 组合).
            # 右键菜单不再显示快捷键提示, 用户自行点击右键菜单项.
            ...
            for action, label, default_key, _cfg_key, _widget_name in self._context_action_defs():
                ...
                record_btn = QPushButton(t("录制"))
                record_btn.setProperty("secondary", True)
                record_btn.setMinimumWidth(80)
                record_btn.clicked.connect(lambda checked=False, action=action: self.record_context_hotkey(action))
                self.ctx_shortcut_buttons[action] = record_btn
                row_layout.addWidget(record_btn)

                clear_btn = QPushButton(t("清除"))
                clear_btn.setProperty("secondary", True)
                clear_btn.setMaximumWidth(60)
                clear_btn.clicked.connect(lambda checked=False, action=action: self.on_context_hotkey_clear(action))
            ...
            menu_shortcut_hint = QLabel(
                t("这些快捷键仅在下方「启用全局热键」开启后注册为系统级热键。") +
                t("右键菜单不再显示快捷键提示，由用户自行点击右键菜单项触发。") +
                t("只支持 Ctrl/Alt/Shift/Win 组合键或受支持的单修饰键。")
            )
            menu_shortcut_hint.setWordWrap(True)
            menu_shortcut_hint.setProperty("muted", True)
            shortcut_layout.addWidget(menu_shortcut_hint)

            global_hotkey_box = QGroupBox(t("全局热键"))
            global_hotkey_layout = QVBoxLayout(global_hotkey_box)
            global_hotkey_layout.setContentsMargins(10, 18, 10, 10)
            ...
            global_hotkey_layout.addWidget(self.global_hotkeys_enabled_check)
            ...
            global_hotkey_layout.addWidget(self.hotkey_focus_guard_check)

            shortcut_hint = QLabel(
                t("开启后，会尝试把上方快捷键注册为系统全局热键；") +
                t("只有 Ctrl / Alt / Shift / Win 组合键或受支持的单修饰键能被系统注册。") + "\n" +
                t("全局热键关闭时，上方快捷键只作为右键菜单显示提示保存。")
            )
            shortcut_hint.setWordWrap(True)
            shortcut_hint.setProperty("muted", True)
            global_hotkey_layout.addWidget(shortcut_hint)
            shortcut_layout.addWidget(global_hotkey_box)

            # v1.4.7: 移除"应用内热键"功能.
            # 原因: ApplicationShortcut 上下文 + 裸键 (R/PgUp/PgDown/Esc) 与 QAction/菜单
            # 快捷键冲突, 打字时触发 activatedAmbiguously 派发, 焦点被抢/应用卡死.
            # 全局热键 (Ctrl+Alt+...) 已足够且不与打字冲突.
            # 保留 self.app_sc_edits / app_sc_enabled_check 为空/None 防止其他代码引用报错.
            self.app_sc_edits = {}
        ...
        # ── 分级重置选项 (v1.4.6) ──
        # 之前只有一个"恢复出厂设置"按钮 (全量重置), 用户反馈希望有"没有那么彻底"的选项.
        # 这里加入 6 个分级重置 + 保留原"恢复出厂设置".
        ...
        granular_hint = QLabel(t("以下选项仅重置对应类别的设置，不影响其它配置。比“恢复出厂设置”更温和。"))
        granular_hint.setWordWrap(True)
        granular_hint.setProperty("muted", True)
        granular_layout.addWidget(granular_hint)
        ...
        def _make_reset_btn(label: str, tooltip: str, handler, danger: bool = False) -> QPushButton:
        ...
        btn_reset_history = _make_reset_btn(
            t("重置壁纸历史"),
            t("清空已切换过的壁纸历史记录（不影响当前壁纸和文件夹）"),
            self._reset_history_only,
        )
        btn_reset_hotkeys = _make_reset_btn(
            t("重置所有快捷键"),
            t("把右键菜单热键和应用内热键恢复为默认值"),
            self._reset_hotkeys_only,
        )
        btn_reset_appearance = _make_reset_btn(
            t("重置外观设置"),
            t("重置主题色、字体路径、程序内 DPI、暗色模式、性能模式"),
            self._reset_appearance_only,
        )
        btn_reset_tray = _make_reset_btn(
            t("重置托盘菜单"),
            t("把托盘菜单项恢复为默认列表"),
            self._reset_tray_only,
        )
        btn_clear_log = _make_reset_btn(
            t("清空实时日志"),
            t("清空内存中的实时日志缓冲区（不影响日志文件）"),
            self._reset_log_buffer_only,
        )
        ...
        def _on_settings_nav(idx):
            ...
            if widget is not None:
                try:
                    ...
                    pass
            # 延迟刷新以避免页面切换动画与 refresh 之间的竞态；只刷新一次。
            # 之前用 singleShot(0, self.refresh_from_config) 会在某些 Qt 版本下
            # 与 QStackedWidget 切换动画产生竞态，导致设置页底部出现短暂白块或
            # 双重滚动条。改为 50ms 延迟并加存活检查，且只刷新当前可见页面。
            def _deferred_refresh():
                if not self._is_qobject_alive(stack):
                    return
                _dlg = getattr(self, "_settings_dialog", None)
                if _dlg is None or not self._is_qobject_alive(_dlg):
                    return
                try:
                    self.refresh_from_config()
                except Exception:
        ...
        nav.currentRowChanged.connect(_on_settings_nav)
        nav.setCurrentRow(0)
        return page
    ...
    def _reset_history_only(self) -> None:
        ...
        try:
            ...
            if reply != QMessageBox.StandardButton.Yes:
                return
            core.config["history"] = []
            core.config["slideshow_last_wallpaper"] = ""
            core.save_config()
    ...
    def _reset_appearance_only(self) -> None:
        ...
        try:
            ...
            for key in ("theme_color", "font_path", "font_weight", "font_size",
                        "dpi_scale", "dark_mode", "performance_mode", "performance_level"):
                if key in defaults:
                    core.config[key] = defaults.get(key)
            core.save_config()
            self._theme_color = core.config.get("theme_color", DEFAULT_THEME_COLOR)
    ...
    def _apply_language_change(self, lang_data: str, source=None):
        ...
        if lang_data != previous_lang:
            ...
            show_info(self, t("提示"), t("界面语言已切换，预览已刷新。"))
    ...
    # v1.4.6: 移除 save_font_settings / save_font_path —— 仅转发到
    # save_display_settings, 无外部调用, 属于死代码.
    ...
    def _bing_tab(self):
        ...
        info = QLabel(t("必应壁纸可同步到本地缓存，也可以直接设置为桌面背景；“继续同步更早壁纸”会从上次记录的位置继续向前获取。"))
        info.setWordWrap(True)
        info.setProperty("muted", True)
        info.setMaximumHeight(48)
        grid.addWidget(info, 0, 0, 1, 7)

        self.bing_cache_edit = QLineEdit(core.config.get("bing_cache_dir", "") or "")
        self.bing_cache_edit.setPlaceholderText(t("请先选择用于保存必应壁纸的目录"))
    ...
    def _about_tab(self):
        ...
        def _build_links_html(fg, lnk):
            ...
            return (
                ...
                + anchor("https://github.com/xxdz-Official/ShangBackground", "xxdz-Official/ShangBackground")
        ...
        # v1.4.7: 系统信息区 — 方便用户报告 bug 时复制环境信息
        ...
        self._sysinfo_label = QLabel(self._build_system_info_text())
        self._sysinfo_label.setWordWrap(True)
        self._sysinfo_label.setTextInteractionFlags(Qt.TextInteractionFlag.TextSelectableByMouse)
        self._sysinfo_label.setStyleSheet("font-family: 'Cascadia Code', 'Consolas', monospace; font-size: 12px;")
        sysinfo_layout.addWidget(self._sysinfo_label)
        copy_sysinfo_btn = QPushButton(t("复制系统信息"))
        copy_sysinfo_btn.setProperty("secondary", True)
        copy_sysinfo_btn.setMaximumWidth(140)
        copy_sysinfo_btn.clicked.connect(self._copy_system_info)
        sysinfo_layout.addWidget(copy_sysinfo_btn, alignment=Qt.AlignLeft)
    ...
    def _build_system_info_text(self) -> str:
        """v1.4.7: 构建系统信息文本 (版本/OS/Python/Qt/mpv)."""
        try:
            import platform
            info_lines = []
            info_lines.append(f"{APP_DISPLAY_NAME} v{APP_VERSION}")
            try:
                from app.version import APP_VERSION_TUPLE
                info_lines.append(f"版本元组: {APP_VERSION_TUPLE}")
            except Exception:
                pass
            info_lines.append(f"操作系统: {platform.system()} {platform.release()} ({platform.machine()})")
            info_lines.append(f"Python: {platform.python_version()}")
            try:
                from PySide6.QtCore import qVersion, __version__ as qt_version
                info_lines.append(f"Qt: {qVersion()} (PySide6 {qt_version})")
            except Exception:
                pass
            try:
                import core
                info_lines.append(f"性能模式: {core.config.get('performance_level', 'balanced')}")
                info_lines.append(f"语言: {core.config.get('language', 'zh')}")
                info_lines.append(f"暗色模式: {core.config.get('dark_mode', False)}")
            except Exception:
                pass
            # mpv 状态
            try:
                ...
                if mpv_path:
                    info_lines.append(f"mpv: {mpv_path}")
                else:
                    info_lines.append("mpv: 未检测到可执行文件；可放入 bin、加入 PATH，或在 Windows 通过 register.bat 注册后再尝试调用。")
            except Exception:
                pass
            return "\n".join(info_lines)
        except Exception as exc:
    ...
    def _copy_system_info(self) -> None:
        """v1.4.7: 复制系统信息到剪贴板, 方便用户报告 bug."""
        try:
            text = self._build_system_info_text()
    ...
    def _log_tab(self):
        ...
        self.log_auto_refresh_check.setChecked(True)
        ...
        # 修复 (v1.4.6): "从文件加载" 会用 setPlainText 覆盖 HTML 渲染并破坏自动刷新,
        # 加 tooltip 说明; 标签改为更清晰的措辞.
    ...
    def eventFilter(self, obj, event):
        if getattr(self, "about_sprite_btn", None) is obj:
            if event.type() == QEvent.Type.Enter:
                ...
                self._fade_about_sprite_to(1)
        # v1.4.7: 移除 PgUp/PgDown eventFilter —— 原本是为应用内热键让 QScrollArea
        # 不消费翻页键, 现在应用内热键已移除, QScrollArea 正常翻页即可.
    ...
    # v1.4.6: 移除 export_log_file —— 死代码, 从未被任何按钮接线.
    # 实际使用的是 export_live_log (实时日志导出).
    ...
    def refresh_from_config(self):
        ...
        try:
            ...
            if hasattr(self, "html_edit"):
                # 从 config 中恢复 HTML 文件或 URL。
                # 与 Linux/MacOS 端对齐：显式 ``or ""`` 兜底，避免配置被破坏
                # （html_file 被显式写成 null）时 setText(None) 抛 TypeError。
            ...
            if hasattr(self, "html_gpu_check"):
                # 默认开启 GPU 加速
                self.html_gpu_check.setChecked(bool(cfg.get("html_gpu_enabled", True)))
            if hasattr(self, "html_mouse_through_check"):
                # 默认开启鼠标穿透（让桌面图标可点击）
                self.html_mouse_through_check.setChecked(bool(cfg.get("html_mouse_through", True)))
            ...
            if hasattr(self, "ctx_shortcut_current_labels"):
                self._refresh_context_shortcut_labels()
            if hasattr(self, "global_hotkeys_enabled_check"):
            ...
            if hasattr(self, "hotkey_focus_guard_check"):
                self.hotkey_focus_guard_check.blockSignals(True)
                self.hotkey_focus_guard_check.setChecked(bool(cfg.get("hotkey_focus_guard", True)))
                self.hotkey_focus_guard_check.blockSignals(False)
    ...
    def update_control_states(self):
        ...
        if hasattr(self, "html_gpu_check"):
            self.html_gpu_check.setEnabled(True)
        if hasattr(self, "html_mouse_through_check"):
            self.html_mouse_through_check.setEnabled(True)
        ...
        if hasattr(self, "html_restart_btn"):
            self.html_restart_btn.setEnabled(is_html and html_running)
    ...
    def on_mode_changed(self, _index=None):
        ...
        if mode_key == "幻灯片放映":
            ...
            video = core.config.get("video_file")
            ...
            path = core.config.get("html_file") or (self.html_edit.text().strip() if hasattr(self, "html_edit") else "")
            if path:
                def _work():
                    core.stop_slideshow()
                    core.stop_video_wallpaper()
                    # start_html_wallpaper 会内部停止已有 HTML 进程
    ...
    def on_seconds_changed(self, value):
        ...
        if normalize_mode_key(core.config.get("mode")) == "幻灯片放映":
            # Only attempt to restart the slideshow when a valid folder has been set.
            # Without a slide_folder configured, restart_slideshow() would return
            # False and the GUI would display a generic "操作失败" error.  This
            # guard avoids spawning an error popup when the user merely adjusts
            # the interval before selecting a folder.
    ...
    def choose_video_file(self):
        ...
        if not path:
            return
        core.config["video_file"] = path
        core.config["mode"] = "视频"
        self.video_edit.setText(path)
        self._set_combo_current_data(self.mode_combo, "视频")
        core.save_config()
        # 选了视频文件后：隐藏引导提示，启用音量滑块（若未静音）
        self._refresh_video_volume_controls()
        def _work():
            core.stop_slideshow()
            core.stop_video_wallpaper()
    ...
    def _html_runtime_options_from_ui(self) -> dict[str, bool]:
        return {
            ...
            "gpu_enabled": bool(self.html_gpu_check.isChecked()) if hasattr(self, "html_gpu_check") else bool(core.config.get("html_gpu_enabled", True)),
            "mouse_through": bool(self.html_mouse_through_check.isChecked()) if hasattr(self, "html_mouse_through_check") else bool(core.config.get("html_mouse_through", True)),
        }

    def _sync_html_runtime_options(self, options: dict[str, bool] | None = None) -> None:
        options = options or {
            "auto_pause": bool(core.config.get("html_auto_pause", True)),
            "gpu_enabled": bool(core.config.get("html_gpu_enabled", True)),
            "mouse_through": bool(core.config.get("html_mouse_through", True)),
        }
        mapping = {
        ...
        for cfg_key, runtime_key in mapping.items():
            try:
                core.html_wallpaper_runtime_set_option(runtime_key, bool(options[cfg_key]))
            except Exception as exc:
    ...
    def _save_html_wallpaper_config_from_ui(self, path: str | None = None) -> str:
        ...
        core.config["html_gpu_enabled"] = options["gpu_enabled"]
        core.config["html_mouse_through"] = options["mouse_through"]
    ...
    def _run_html_wallpaper_from_gui(self, path: str | None = None, *, restart: bool = False, status_text: str | None = None) -> None:
        target = self._save_html_wallpaper_config_from_ui(path)
        if not target:
            show_info(self, t("HTML 壁纸"), t("请先选择 HTML 文件或输入网址"))
            return
    ...
    def start_html_wallpaper_from_gui(self):
        """在图形界面中启动 HTML 壁纸。"""
        self._run_html_wallpaper_from_gui(self._html_current_path_from_ui())

    def restart_html_wallpaper_from_gui(self):
        """手动重启 HTML 壁纸（应用 GPU 切换等无法热更新的设置）。"""
        if not core.is_html_wallpaper_running():
            self.start_html_wallpaper_from_gui()
            return
        target = self._html_current_path_from_ui() or core.html_wallpaper_get_last_path()
        self._run_html_wallpaper_from_gui(target, restart=True)
    ...
    def on_html_gpu_changed(self, checked):
        """GPU 加速选项切换：写盘并在运行时重启 HTML 子进程。"""
        self._set_html_runtime_option_from_gui(
            "html_gpu_enabled",
            "gpu_enabled",
            checked,
            restart_if_running=True,
            restart_status=t("正在重启 HTML 壁纸以应用 GPU 设置…"),
        )

    def on_html_mouse_through_changed(self, checked):
        """鼠标穿透选项切换：写盘 + 热通知子进程。"""
        self._set_html_runtime_option_from_gui("html_mouse_through", "mouse_through", checked)

    def start_video_wallpaper_from_gui(self):
        path = (self.video_edit.text().strip() if hasattr(self, "video_edit") else "") or core.config.get("video_file", "")
        if not path:
            show_info(self, t("视频壁纸"), t("请先选择视频文件"))
            return
        core.config["video_file"] = path
        core.config["mode"] = "视频"
        self._set_combo_current_data(self.mode_combo, "视频")
    ...
    def _restore_video_focus_policy(self) -> None:
        try:
            if getattr(self, "_video_focus_paused", False):
                core.set_video_paused(False)
                self._video_focus_paused = False
            if getattr(self, "_video_focus_ducked", False):
                core.set_video_volume(bool(core.config.get("video_muted", True)), int(core.config.get("video_volume", 100)))
                self._video_focus_ducked = False
        except Exception:
            pass
    ...
    def _apply_video_focus_policy_tick(self) -> None:
        try:
            ...
            if on_desktop:
                ...
                return
            if policy == "pause":
                if not getattr(self, "_video_focus_paused", False):
                ...
                if getattr(self, "_video_focus_paused", False):
                    core.set_video_paused(False)
                    self._video_focus_paused = False
                if not bool(core.config.get("video_muted", True)) and not getattr(self, "_video_focus_ducked", False):
                    ...
                    if core.set_video_volume(False, duck):
                        self._video_focus_ducked = True
        except Exception:
            pass

    def _refresh_video_volume_controls(self):
        """根据当前 config 同步音量滑块的启用状态与引导提示可见性。

        滑块启用条件：(视频模式 OR 已选视频文件) AND 未静音
        引导提示可见条件：未选视频文件
        """
    ...
    def on_video_muted_changed(self, checked):
        ...
        if normalize_mode_key(core.config.get("mode")) == "视频" and core.config.get("video_file"):
            ...
            if core.is_video_wallpaper_running():
                if core.set_video_volume(bool(checked), volume):
                    # IPC 热更新成功，无需重启播放进程
                    return
            # IPC 不可用或失败：回退到重启
    ...
    def _apply_video_volume_live(self):
        ...
        volume = int(core.config.get("video_volume", 100))
        muted = bool(core.config.get("video_muted", True))
        if core.set_video_volume(muted, volume):
            return  # IPC 热更新成功
    ...
    # v1.4.7 修复: 恢复 _context_hotkey / _context_hotkey_display /
    # _context_checkbox_label / _refresh_context_shortcut_labels 四个方法.
    # 这些方法在 v1.4.6 应用内热键移除时被误删 (它们属于右键菜单热键, 不属于应用内热键),
    # 导致 _wallpaper_tab 第 1912 行调用 _refresh_context_shortcut_labels 时 AttributeError 崩溃.
    ...
    # v1.4.6: 移除 on_context_hotkey_changed / on_context_hotkey_recorded ——
    # QKeySequenceEdit 旧路径残留, 当前录制走 pynput (record_context_hotkey),
    # 这两个方法无任何调用点.
    ...
    def on_hotkey_focus_guard_changed(self, checked: bool) -> None:
        """Enable/disable focus-position guard for global hotkeys."""
    ...
    def open_global_settings_from_home(self):
        ...
        dialog = QDialog(self)
        dialog.setObjectName("GlobalSettingsDialog")
        ...
        # Fix 1.6 / 3.3: 关闭设置对话框时，把所有 editingFinished-only 的编辑
        # 项强制 flush 到 config（应用内快捷键、字体路径等），避免用户没按 Enter
        # 就关闭对话框导致改动丢失。同时如果对话框有 Esc 快捷键绑定，覆盖
        # QDialog 默认 reject 行为以让 Esc 转发给应用内快捷键系统。
        def _on_settings_close(event):
            try:
                ...
                try:
                    ...
                    pass
            super(QDialog, dialog).closeEvent(event)
        dialog.closeEvent = _on_settings_close
        dialog.show()
        dialog.raise_()
    ...
    def _reset_settings_dialog_to_initial_position(self) -> None:
        ...
        try:
            ...
            if not self._is_qobject_alive(dlg):
                return
            # 1. 重置左侧导航选中第一项 (属性名是 _settings_nav)
    ...
    def _flush_settings_dialog_unsaved_changes(self):
        ...
        if self._is_qobject_alive(font_path_edit):
            try:
                ...
                if current_font and current_font != saved_font:
    ...
    def show_auto_start_prompt(self):
        ...
        # NOTE: Do NOT call dialog.setStyleSheet(self._theme_stylesheet) here.
        # The application-level QSS installed by _rebuild_stylesheet already
        # cascades to every child dialog; copying the theme string onto
        # individual instances was a historic source of style drift.
    ...
    def _refresh_favorites_list(self) -> None:
        ...
        try:
            ...
            for path in core.config.get("favorites", [])[:50]:
            ...
            try:
                core.log(f"刷新收藏夹失败: {exc}", level="WARNING")
            except Exception:
                pass
    ...
    def _toggle_favorite_current(self) -> None:
        ...
        try:
            ...
            if not path or not os.path.exists(path):
                ...
                return
            favs = list(core.config.get("favorites", []))
            if path in favs:
                favs.remove(path)
                core.config["favorites"] = favs
                core.save_config()
                self._refresh_favorites_list()
                self._update_favorite_button_state()
    ...
    def _update_favorite_button_state(self) -> None:
        ...
        try:
            ...
            favs = core.config.get("favorites", [])
            if path and path in favs:
    ...
    def _apply_favorite_item(self, item) -> None:
        ...
        try:
            ...
            if path and os.path.exists(path):
                def _apply_from_fav():
                    return core.set_wallpaper(path, t("收藏夹"))
                self.run_core(_apply_from_fav)
        except Exception as exc:
            try:
        ...
        try:
            ...
            if action == act_remove:
                try:
                    ...
                    core.log(f"移除收藏失败: {exc}", level="WARNING")
    ...
    def apply_history_item(self, item: QListWidgetItem):
        ...
        if path and os.path.exists(path):
            def _apply_from_history():
                return core.set_wallpaper(path, t("历史记录"))
    ...
    def use_bing_cache_as_slideshow(self):
        ...
        if not folder or not os.path.isdir(folder):
            show_warning(self, t("必应壁纸"), t("请先选择或同步一个有效的缓存目录。"))
            return
        core.config["slide_folder"] = folder
        core.config["mode"] = "幻灯片放映"
        self.folder_edit.setText(folder)
    ...
    def sync_bing_wallpaper(self, set_latest: bool = True, continue_from_saved: bool = False, force_count: int | None = None):
        cache_dir = self.bing_cache_edit.text().strip()
        if not cache_dir:
            show_info(self, t("必应壁纸"), t("首次使用必应壁纸前，请先选择或填写缓存目录。"))
    ...
    def show_about_dialog(self):
        ...
        def _build_about_dialog_links_html(fg, lnk):
            return (
                f'<span style="color:{fg}">原项目：</span>'
                f'<a href="https://github.com/xxdz-Official/ShangBackground" style="color:{lnk}">GitHub</a><br>'
    ...
    def _perform_exit_cleanup_once(self, *, restore_wallpaper: bool = True):
        if getattr(self, "_exit_cleanup_done", False):
            return
        self._exit_cleanup_done = True
        self._disconnect_own_signals()
        try:
            core.request_cancel_operations(t("程序退出"))
        except Exception:
            pass
        try:
            core.stop_slideshow()
        except Exception as exc:
            core.log(f"停止幻灯片失败: {exc}")
        try:
            core.stop_video_wallpaper()
        except Exception as exc:
            core.log(f"停止视频壁纸失败: {exc}")
        # 退出时也停止 HTML 动态壁纸，防止退出后仍在运行
        try:
            core.stop_html_wallpaper()
        except Exception as exc:
            core.log(f"停止 HTML 壁纸失败: {exc}")
        # 退出时注销所有全局热键
        try:
            core.stop_global_hotkeys()
        except Exception as exc:
            core.log(f"停止全局热键失败: {exc}")
        if restore_wallpaper:
            try:
                core.restore_session_original_wallpaper()
            except Exception as exc:
                core.log(f"恢复启动前壁纸失败: {exc}")
        try:
    ...
    def exit_app(self):
        ...
        if self.tray:
            ...
            QApplication.processEvents()
            self._refresh_shell_ui_later()
        self._perform_exit_cleanup_once(restore_wallpaper=True)
